...
import com.lexing.batterytest.Utils;
...
public class BatteryStatsHelper {
	...
	private IBatteryStatsV18 mBatteryStatsWrapper;
	private int mStatsType = BatteryStatsImplV18.STATS_SINCE_UNPLUGGED;
	private PowerProfileV18 mPowerProfile;
	private static BatteryStatsImplV18 mStats;
	...
	public BatteryStatsHelper(Context context) {
		testType = 1;
		mContext = context;
		mBatteryStatsWrapper = IBatteryStatsV18.create();
		mPowerProfile = PowerProfileV18.create(context);
	}
	...
	private void addPhoneUsage(long uSecNow) {
		...
		double phoneOnPower = mPowerProfile.getAveragePower(PowerProfileV18.POWER_RADIO_ACTIVE) * phoneOnTimeMs / 1000;
	...
	private void addScreenUsage(long uSecNow) {
		...
		power += screenOnTimeMs * mPowerProfile.getAveragePower(PowerProfileV18.POWER_SCREEN_ON);
		final double screenFullPower = mPowerProfile.getAveragePower(PowerProfileV18.POWER_SCREEN_FULL);
		for (int i = 0; i < BatteryStatsImplV18.NUM_SCREEN_BRIGHTNESS_BINS; i++) {
			double screenBinPower = screenFullPower * (i + 0.5f) / BatteryStatsImplV18.NUM_SCREEN_BRIGHTNESS_BINS;
	...
	private void addWiFiUsage(long uSecNow) {
		...
		if (runningTimeMs < 0)
			runningTimeMs = 0;
		double wifiPower = (onTimeMs * 0 * mPowerProfile.getAveragePower(PowerProfileV18.POWER_WIFI_ON) + runningTimeMs
				* mPowerProfile.getAveragePower(PowerProfileV18.POWER_WIFI_ON)) / 1000;
		if (DEBUG)
	...
	private void addBluetoothUsage(long uSecNow) {
		...
		double btPower = btOnTimeMs * mPowerProfile.getAveragePower(PowerProfileV18.POWER_BLUETOOTH_ON) / 1000;
		int btPingCount = mStats.getBluetoothPingCount();
		btPower += (btPingCount * mPowerProfile.getAveragePower(PowerProfileV18.POWER_BLUETOOTH_AT_CMD)) / 1000;
	...
	private void addIdleUsage(long uSecNow) {
		...
		double idlePower = (idleTimeMs * mPowerProfile.getAveragePower(PowerProfileV18.POWER_CPU_IDLE)) / 1000;
		addEntry(BatterySipper.DrainType.IDLE, idleTimeMs, idlePower);
	}

	private void addRadioUsage(long uSecNow) {
		double power = 0;
		final int BINS = BatteryStatsImplV18.NUM_SIGNAL_STRENGTH_BINS;
		long signalTimeMs = 0;
		for (int i = 0; i < BINS; i++) {
			long strengthTimeMs = mStats.getPhoneSignalStrengthTime(i, uSecNow, mStatsType) / 1000;
			power += strengthTimeMs / 1000 * mPowerProfile.getAveragePower(PowerProfileV18.POWER_RADIO_ON, i);
			signalTimeMs += strengthTimeMs;
		}
		long scanningTimeMs = mStats.getPhoneSignalScanningTime(uSecNow, mStatsType) / 1000;
		power += scanningTimeMs / 1000 * mPowerProfile.getAveragePower(PowerProfileV18.POWER_RADIO_SCANNING);
	...
	private void processAppUsage() {
		...
		speedSteps = mPowerProfile.getNumSpeedSteps();
		final double[] powerCpuNormal = new double[speedSteps];
		final long[] cpuSpeedStepTimes = new long[speedSteps];
		for (int p = 0; p < speedSteps; p++) {
			powerCpuNormal[p] = mPowerProfile.getAveragePower(PowerProfileV18.POWER_CPU_ACTIVE, p);
		}

		long uSecTime = mStats.computeBatteryRealtime(SystemClock.elapsedRealtime() * 1000, which);
		mStatsPeriod = uSecTime;
		SparseArray<? extends BatteryStatsImplV18.UidWrapper> uidStats = mStats.getUidStats();
		final int NU = uidStats.size();
		for (int iu = 0; iu < NU; iu++) {
			BatteryStatsImplV18.UidWrapper u = uidStats.valueAt(iu);
			double power = 0;
			double highestDrain = 0;
			String packageWithHighestDrain = null;
			Map<String, ? extends BatteryStatsImplV18.UidWrapper.ProcWrapper> processStats = u.getProcessStats();
			long cpuTime = 0;
			long cpuFgTime = 0;
			long wakelockTime = 0;
			long gpsTime = 0;
			if (processStats.size() > 0) {
				// 1, Process CPU time
				for (Map.Entry<String, ? extends BatteryStatsImplV18.UidWrapper.ProcWrapper> ent : processStats.entrySet()) {
					if (DEBUG)
						Log.i(TAG, "Process name = " + ent.getKey());

					BatteryStatsImplV18.UidWrapper.ProcWrapper ps = ent.getValue();
			...
			Map<String, ? extends BatteryStatsImplV18.UidWrapper.WakelockWrapper> wakelockStats = u.getWakelockStats();
			for (Map.Entry<String, ? extends BatteryStatsImplV18.UidWrapper.WakelockWrapper> wakelockEntry : wakelockStats.entrySet()) {
				BatteryStatsImplV18.UidWrapper.WakelockWrapper wakelock = wakelockEntry.getValue();
				// Only care about partial wake locks since full wake locks are canceled when the user turns the screen off.
				BatteryStatsImplV18.TimerWrapper timer = wakelock.getWakeTime(BatteryStatsImplV18.WAKE_TYPE_PARTIAL);
				if (timer != null) {
			...
			power += (wakelockTime * mPowerProfile.getAveragePower(PowerProfileV18.POWER_CPU_AWAKE)) / 1000;

			// 3, Add cost of data traffic
			final double averageCostPerByte;

			averageCostPerByte = getAverageDataCost();
			...
			if (versionValid()) {
				...
				power += (wifiRunningTimeMs * mPowerProfile.getAveragePower(PowerProfileV18.POWER_WIFI_ON)) / 1000;
			}

			// 5, Process Sensor usage
			Map<Integer, ? extends BatteryStatsImplV18.UidWrapper.SensorWrapper> sensorStats = u.getSensorStats();
			for (Map.Entry<Integer, ? extends BatteryStatsImplV18.UidWrapper.SensorWrapper> sensorEntry : sensorStats.entrySet()) {
				BatteryStatsImplV18.UidWrapper.SensorWrapper sensor = sensorEntry.getValue();
				int sensorType = sensor.getHandle();
				BatteryStatsImplV18.TimerWrapper timer = sensor.getSensorTime();
				long sensorTime = timer.getTotalTimeLocked(uSecTime, which) / 1000;
				double multiplier = 0;
				switch (sensorType) {
					case BatteryStatsImplV18.UidWrapper.SensorWrapper.GPS:
						multiplier = mPowerProfile.getAveragePower(PowerProfileV18.POWER_GPS_ON);
						gpsTime = sensorTime;
						break;
					default:
			...
			if (power != 0) {
				BatterySipper app = new BatterySipper(mContext, BatterySipper.DrainType.APP, u, new double[] { power });
				app.cpuTime = cpuTime;
				app.gpsTime = gpsTime;
				...
				} else if (u.getUid() == BLUETOOTH_UID) {
	...
	private double getAverageDataCost() {
		...
		final double WIFI_POWER = mPowerProfile.getAveragePower(PowerProfileV18.POWER_WIFI_ACTIVE) / 3600;
		final double MOBILE_POWER = mPowerProfile.getAveragePower(PowerProfileV18.POWER_RADIO_ACTIVE) / 3600;
	...
	private BatteryStatsImplV18 load() {
		BatteryStatsImplV18 stats = null;
		try {
			byte[] data = mBatteryStatsWrapper.getStatistics();
			Parcel parcel = Parcel.obtain();
			parcel.unmarshall(data, 0, data.length);
			parcel.setDataPosition(0);
			stats = BatteryStatsImplV18.create(parcel);
			if (versionValid()) {
				stats.distributeWorkLocked(BatteryStatsImplV18.STATS_SINCE_CHARGED);
			}
